...
find "$OUTPUT" -maxdepth 1 -type f \( -name '*.deb' -o -name '*.rpm' \) -print0 | while IFS= read -r -d '' file; do gpg --armor --detach-sign --local-user 'OldChat For AllPlatform Release' "$file"; done
