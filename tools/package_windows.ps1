...
if (-not (Test-Path -LiteralPath $flutterBundle -PathType Container)) { throw "Flutter release bundle was not produced: $flutterBundle" }
...
if ($builtExecutables.Count -ne 1) { throw "Expected exactly one executable in $outputPath, found $($builtExecutables.Count)" }
$targetExecutable = Join-Path $outputPath $executableName
if ($builtExecutables[0].FullName -ne $targetExecutable) { Move-Item -LiteralPath $builtExecutables[0].FullName -Destination $targetExecutable -Force }
