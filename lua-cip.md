...
- Windows 能力必须在 manifest 声明；`camera` 只能请求用户主动拍照，`files.local` 只能读取用户在系统选择器中明确选中的普通文件。
- 插件不能扫描目录、读取任意路径、执行 Shell/PowerShell/EXE 或访问登录凭据。
...
| `order` | 否 | 数字越小越靠前 |
| `permissions` | 是 | 只允许 `network`、`network_external`、`storage`、`storage.local`、`camera`、`files.local` |
