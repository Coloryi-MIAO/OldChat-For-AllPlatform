...
class ImageCacheService {
  ...
  ImageProvider provider(String url, {int cacheWidth = 512}) {
    ...
    final headers =
        normalized.contains('/channel-media/') || token == null || token.isEmpty
        ? null
  ...
  Future<File?> cachedFile(String url) async {
    ...
    try {
      ...
      if (identical(_downloads[normalized], future))
        _downloads.remove(normalized);
    }
  }
  ...
  Future<File?> _download(String normalized) async {
    ...
    final directoryPath = await CacheService().cacheDirectory(userId: uid);
    final file = File(
      '$directoryPath${Platform.pathSeparator}media_${_key(normalized)}',
    );
    final token = AuthService().token;
    final headers = <String, String>{
      ...
      if (token != null &&
          token.isNotEmpty &&
          !normalized.contains('/channel-media/'))
        'Authorization': 'Bearer $token',
    };
    Object? lastError;
    for (final candidate
        in candidates.isEmpty ? <String>[normalized] : candidates) {
      try {
  ...
  Future<File?> _findExisting(String url) async {
    ...
    for (final candidate in <String>[
      normalized,
      ...resolveMediaCandidates(normalized),
    ].toSet()) {
      final file = File(
        '$directoryPath${Platform.pathSeparator}media_${_key(candidate)}',
      );
